...
module.exports.run = async (bot, message, args) => {
    ...
    message.delete().then(async () => {
        let dungeonMasterIds = await getAllDungeonMasterIds();
        const sessionChannelIdArray = await collectAllSessionChannelIds();
        ...
        if (message.member.roles.cache.has(message.guild.roles.cache.find(role => role.name === 'Dungeon Master').id) && message.mentions.users.first()) {
            if (!dungeonMasterIds.includes(message.mentions.users.first().id)) {
                user = message.mentions.users.first();
            } else return message.channel.send('Dungeon Masters can not leave sessions!').then(msg => msg.delete({ timeout: 5000 })).catch(err => console.log(err));
        } else if (message.member.roles.cache.has(message.guild.roles.cache.find(role => role.name === 'Dungeon Master').id)) return message.channel.send('Dungeon Masters can not leave sessions!').then(msg => msg.delete({ timeout: 5000 })).catch(err => console.log(err));

        let sessionParty = session.get('session_party');
        if (sessionParty.includes(user.id)) {
...
async function getAllDungeonMasterIds() {
    let dungeonMasterIds = []
    await DungeonMaster.findAll().then(dungeonMasters => {
...
async function collectAllSessionChannelIds() {
    let sessionChannelIdArray = [];
    await SessionRequest.findAll().then(sessionRequest => {
        sessionRequest.forEach(request => {
            sessionChannelIdArray.push(request.get('session_channel_id'));
        });
    });
    await PlannedSession.findAll().then(plannedSession => {
