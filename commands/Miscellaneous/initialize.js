    ...
    if (args[0]) {
        ...
        if (args[0].toUpperCase() === "verify".toUpperCase()) {
            ...
            message.guild.channels.create('--VERIFICATION--', {
                ...
                const verifyChannel = createChannel(`verify`, "text", "--VERIFICATION--", bot).then(channel => {
                    channel.send(verifyMessage).then(message => {
                        message.react('✅');
                    });
                });
            });
...
async function createChannel(name, category, categoryParentName, bot) {
    let serverStatCategory = bot.channels.cache.find(c => c.name == categoryParentName && c.type == "category");
